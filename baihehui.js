...
class Baihehui extends ComicSource {
  ...
  // update url
  url = "https://cdn.jsdelivr.net/gh/haukuen/venera-configs@main/baihehui.js";

  settings = {
    domains: {
      title: "主页源",
      type: "select",
      options: [{ value: "yamibo.com" }],
      default: "yamibo.com",
    },
  };
  ...
  account = {
    login: async (username, password) => {
      ...
      let sc1 =
        resGet.headers["set-cookie"] || resGet.headers["Set-Cookie"] || [];
      let initialCookies = [];
      for (let line of Array.isArray(sc1) ? sc1 : [sc1]) {
        ...
        if (name === "PHPSESSID" || name === "_csrf-frontend") {
          initialCookies.push(
            new Cookie({ name, value, domain: "www.yamibo.com" }),
          );
        }
      }
      ...
      let csrf = doc.querySelector('meta[name="csrf-token"]').attributes
        .content;
      doc.dispose();
      ...
      let cookies = parts
        .map((line) => {
          ...
          v = v.trim();
          if (names.includes(k))
            return new Cookie({ name: k, value: v, domain: "www.yamibo.com" });
        })
        .filter(Boolean);
      ...
      if (cookies.length !== names.length) {
        throw (
          "登录未返回完整 Cookie，实际：" + cookies.map((c) => c.name).join(",")
        );
      }
  ...
  explore = [
    {
      ...
      load: async (page) => {
        ...
        function parseItem(el) {
          let a =
            el.querySelector(".media-img") || el.querySelector("a.media-img");
        ...
        // 4. 抓「编辑推荐」
        let editor = [];
        let editorEls = doc.querySelectorAll(
          ".recommend-list .media-cell.horizontal",
        );
        ...
        let latestTitle = doc
          .querySelectorAll("h2.module-title")
  ...
  categoryComics = {
    load: async (category, params, options, page) => {
      ...
      let maxPage = lastPageElement
        ? parseInt(lastPageElement.attributes["data-page"]) + 1
        : 1;
  ...
  comic = {
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      ...
      // 提取评论总数
      let totalCommentsMatch = document
        .querySelector("div.panel-body")
        .text.match(/共(\d+)篇/);
      let totalComments = totalCommentsMatch
        ? parseInt(totalCommentsMatch[1])
        : 0;
      ...
      document.querySelectorAll("div.post.row").forEach((post) => {
        ...
        let avatar =
          ...
          post.querySelector("a > img.cmt-avatar").attributes["src"];
        let content = post.querySelector("div.row > p").text.trim();
        let time = post
          .querySelector("span.description")
          .text.replace("在 ", "")
          .trim();
        let replyCountMatch = post
          .querySelector("a.btn.btn-sm")
          .text.match(/(\d+) 条回复/);
      ...
      let maxPage = maxPageElement
        ? parseInt(maxPageElement.attributes["data-page"]) + 1
        : 1;
